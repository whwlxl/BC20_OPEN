...
*               Input "DEBUGEnable"  to enable  debug  port  output nema statement.
*               Input "DEBUGDisable"  to disable  debug  port  output nema statement.
...
static void Callback_UART_Hdlr(Enum_SerialPort port, Enum_UARTEventType msg, bool level, void* param)
{
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            if(p)
            {
                iRet = RIL_GPS_Open(GPS_POWER_MODE_ON);
            ...
            if(p)
            {
                iRet = RIL_GPS_Open(GPS_POWER_MODE_OFF);
                if(RIL_AT_SUCCESS != iRet) 
                {
            ...
            /* Power On GPS */
			p = Ql_strstr(m_RxBuf_Uart, "DEBUGEnable");
            if(p)
            {
                iRet = RIL_GPS_DEBUG_Enable(GPS_DEBUG_MODE_ENABLE);
                if(RIL_AT_SUCCESS != iRet) 
                {
                    APP_DEBUG("Power on GPS fail, iRet = %d.\r\n", iRet);
                    break;
            ...
            /* Power Off GPS */
            p = Ql_strstr(m_RxBuf_Uart, "DEBUGDisable");
            if(p)
            {
                iRet = RIL_GPS_Open(GPS_DEBUG_MODE_DISABLE);
                if(RIL_AT_SUCCESS != iRet) 
                {
                    APP_DEBUG("Power off GPS fail, iRet = %d.\r\n", iRet);
                    break;
