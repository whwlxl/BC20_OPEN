...
void spi_flash_cs(bool CS)
{
	...
	{
		if (CS)
			Ql_GPIO_SetLevel(PINNAME_GPIO11,PINLEVEL_HIGH);
		else
			Ql_GPIO_SetLevel(PINNAME_GPIO11,PINLEVEL_LOW);
	}
}
...
static void spi_init(u8 spi_type)
{
    ...
    ret = Ql_SPI_Init(USR_SPI_CHANNAL,PINNAME_GPIO11,PINNAME_GPIO13,PINNAME_GPIO12,PINNAME_GPIO14,spi_type);
    ...
    if (!spi_type)
    {
    	Ql_GPIO_Init(PINNAME_GPIO11,PINDIRECTION_OUT,PINLEVEL_HIGH,PINPULLSEL_PULLUP);	 //CS high
    }
}
